...
def cpm_timeline_calendar(project: str | None = None):
	...
	out = []
	for row in items:
		base = base_dates.get(row["project"]) or fallback
...
def _project_base_dates(project: str | None = None) -> dict[str, object]:
	...
	if project and project not in base_dates:
		contract_start = frappe.db.get_value("Project Contract", project, "planned_start")
		base_dates[project] = getdate(contract_start) if contract_start else getdate()
...
def preview_sector_kpi(scenario: str | None = None, params: str | None = None) -> dict:
	...
	return _core_preview("projects_pm", scenario=scenario, params=params)
