...
frappe.pages["pm_schedule_gantt"].on_page_load = function (wrapper) {
	...
	if (frappe.route_options && frappe.route_options.project) {
		project_control.set_value(frappe.route_options.project);
	}

	page.set_primary_action(__("Load schedule"), () => load_schedule());
	page.set_secondary_action(__("Sync WBS from BOQ"), () => sync_wbs());
	...
	function sync_wbs() {
		...
		frappe.call({
			...
			args: { project_contract: project, chain_dependencies: 1 },
			freeze: true,
			callback(r) {
				const m = r.message || {};
				frappe.show_alert({
					message: __("WBS synced: {0} task(s)", [m.tasks || 0]),
					indicator: "green",
				});
				load_schedule();
			},
		...
		if (!items.length) {
			$chart.append(
				`<p class="text-muted">${__(
					"No timeline rows. Select a project and click Sync WBS from BOQ, or add PM WBS tasks with planned dates."
				)}</p>`
			);
			return;
		}
